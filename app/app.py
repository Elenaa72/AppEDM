...
def cargar_geojson():
    ...
    for feature in data['features']:
        feature['properties']['nombre'] = feature['properties']['nombre'].upper().strip()
    return data

geojson_data = cargar_geojson()

# Preparar tabla_pct para merge con geojson
tabla_pct = tabla_pct.reset_index()
tabla_pct['barrio_localizacion'] = tabla_pct['barrio_localizacion'].str.upper().str.strip()

# Crear un diccionario de clusters por barrio para asignar al geojson
cluster_dict = dict(zip(tabla_pct['barrio_localizacion'], tabla_pct['cluster']))

# Asignar cluster a cada feature del geojson
for feature in geojson_data['features']:
    barrio = feature['properties']['nombre']
    feature['properties']['cluster'] = cluster_dict.get(barrio, None)
    # Para mostrar +1 en tooltip o N/A
    feature['properties']['cluster_display'] = (feature['properties']['cluster'] + 1) if feature['properties']['cluster'] is not None else 'N/A'

# Paleta colores para los clusters
