...
fig, ax = plt.subplots(figsize=(7, 4))  # Tamaño ajustado
sns.barplot(
...
@st.cache_data
def cargar_geojson():
    return gpd.read_file("data/barris-barrios.geojson")

gdf = cargar_geojson()
# Normalizar nombres para merge (mayúsculas y sin espacios)
gdf['barrio'] = gdf['nombre'].str.upper().str.strip()
tabla_pct = tabla_pct.reset_index()
tabla_pct['barrio_localizacion'] = tabla_pct['barrio_localizacion'].str.upper().str.strip()

# Merge para asignar cluster a barrios geojson
gdf = gdf.merge(tabla_pct[['barrio_localizacion', 'cluster']], left_on='barrio', right_on='barrio_localizacion', how='left')

# Paleta colores para los 4 clusters
...
def style_function(feature):
    ...
    if cluster is None or pd.isna(cluster):
        return {
            'fillColor': '#8c8c8c',  # gris para sin cluster
            'color': 'black',
            'weight': 0.5,
            'fillOpacity': 0.5
        }
...
# Creamos campo para mostrar cluster + 1 en tooltip (si hay cluster)
gdf['cluster_display'] = gdf['cluster'].apply(lambda x: int(x)+1 if pd.notna(x) else 'N/A')

tooltip = GeoJsonTooltip(
    fields=['barrio', 'cluster_display'],
